...
from opsvis.settings import *
from opsvis import model as opsvmodel
...
def _plot_defo_mode_2d(modeNo, sfac, nep, unDefoFlag, fmt_defo, fmt_undefo,
    ...
    for i, ele_tag in enumerate(ele_tags):
        ...
        # beam/frame element plot_defo
            ...
            ax.plot([ecrd_nodes[1, 0] + sfac * ed[1, 0],
                    ...
                    **fmt_model_rigid_offset)
            np.append(x, ecrd_nodes[0, 0] + sfac * ed[0, 0])
            np.append(x, ecrd_nodes[0, 1] + sfac * ed[0, 1])
            np.append(x, ecrd_nodes[1, 0] + sfac * ed[1, 0])
            np.append(x, ecrd_nodes[1, 1] + sfac * ed[1, 1])

            np.append(y, ecrd_nodes[0, 0] + sfac * ed[0, 0])
            np.append(y, ecrd_nodes[0, 1] + sfac * ed[0, 1])
            np.append(y, ecrd_nodes[1, 0] + sfac * ed[1, 0])
            np.append(y, ecrd_nodes[1, 1] + sfac * ed[1, 1])
    ...
    return ax, ycdi, 0, total_x, total_y
...
def plot_defo(sfac=False, nep=17, unDefoFlag=1, fmt_defo=fmt_defo,
    ...
    if ndim == 2:
        ...
        ax, min_defl, max_defl, x, y = _plot_defo_mode_2d(0, sfac, nep, unDefoFlag, fmt_defo,
    ...
    return sfac, ax, min_defl, max_defl, x, y
...
def plot_mode_shape(modeNo, sfac=False, nep=17, unDefoFlag=1,
    ...
    if ndim == 2:
        ...
        ax, min_defl, max_defl = _plot_defo_mode_2d(modeNo, sfac, nep, unDefoFlag,
